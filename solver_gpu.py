...
class Solver(object):
    ...
    def grad_tomo(self, xi0, xi1, xi2, niter, init, rho, tau, eta):
        ...
        for i in range(niter):
            ...
            res = res - 2*eta*r*r*(rho*tmp0-tau*tmp1)
    ...
    def admm(self, data, h, psi, phi, lamd, mu, x, rho, tau, alpha, gamma, eta, piter, titer, NITER):
        res = np.zeros([NITER, 4], dtype="float32")
        for m in range(NITER):
            # keep previous iteration
            psi0, phi0, x0, h0, lamd0, mu0 = psi, phi, x, h, lamd, mu
            ...
            if (np.mod(m, 1) == 0):
                ...
                print("%d) rho=%f Lagrangian terms:  %.2e %.2e %.2e %.2e %.2e %.2e %.2e" %
                      (m, rho, res[0], res[1], res[2], res[3], res[4], res[5], res[6]))
