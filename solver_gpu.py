...
class Solver(object):
    def __init__(self, prb, scan, theta, det, voxelsize, energy, tomoshape):
        ...
        self.cl_ptycho = ptychofft.ptychofft(
            *self.tomoshape, *self.ptychoshape[1:], prb.shape[0])
    ...
    def cg_tomo(self, xi0, xi1, K, niter, init, rho, tau):
        ...
        def minf(KRu, gu): return rho*np.linalg.norm(KRu-xi0)**2 + \
            tau*np.linalg.norm(gu-xi1)**2
        u = init
        gamma = 8  # init gamma as a large value
        for i in range(niter):
            KRu = K*self.fwd_tomo(u)
            gu = self.fwd_reg(u)
            grad = rho*self.adj_tomo(np.conj(K)*(KRu-xi0)) + \
    ...
    def cg_poisson_ptycho(self, data, init, niter, rho, h, lamd):
        def minf(psi, fpsi): return np.sum(np.abs(fpsi)**2-2*data*np.log(np.abs(fpsi) +
                                                                         (data < 1e-5).astype("float32"))) + rho*np.linalg.norm(h-psi+lamd/rho)**2
        psi = init.copy()
        gamma = 8  # init gamma as a large value
        for i in range(niter):
            fpsi = self.fwd_ptycho(psi)
    ...
    def take_lagr(self, psi, phi, data, h, e, lamd, mu, tau, rho, alpha, model):
        ...
        fpsi = self.fwd_ptycho(psi)
        if (model == 'poisson'):
            lagr[0] = np.sum(np.abs(fpsi)**2-2*data *
                             np.log(np.abs(fpsi)+(data < 1e-5).astype("float32")))
        if (model == 'gaussian'):
        ...
        for m in range(NITER):
            ...
            if (np.mod(m, 4) == 0):
                lagr[m] = self.take_lagr(
                    psi, phi, data, h, e, lamd, mu, tau, rho, alpha, model)
