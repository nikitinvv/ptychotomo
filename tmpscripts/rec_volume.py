...
if __name__ == "__main__":
    ...
    name = 'reg'+str(alpha)+'noise'+str(noise)+'maxint' + \
        str(maxint)+'prbshift'+str(prbshift)+'ntheta'+str(ntheta)+str(model)+str(piter)+str(titer)+str(NITER)+'p'+str(0)

    dxchange.write_tiff(u.imag.get(),  '/data/staff/tomograms/viknik/beta/beta'+name)
    dxchange.write_tiff(u.real.get(),  '/data/staff/tomograms/viknik/delta/delta'+name)
