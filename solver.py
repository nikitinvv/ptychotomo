...
class Solver(object):
    ...
    def logtomo(self, psi):
        return -1j / self.wavenumber() * cp.log(psi) / self.voxelsize*self.coeftomo
    ...
    def line_search(self, minf, gamma, u, fu, d, fd):
        while(minf(u, fu)-minf(u+gamma*d, fu+gamma*fd) < 0 and gamma > 1e-10):
            gamma *= 0.5
        if(gamma <= 1e-10):# direction not found  
            gamma = 0
        return gamma
    ...
    def cg_ptycho(self, data, init, h, lamd, rho, piter, model):
        ...
        def minf(psi, fpsi):
            if model == 'gaussian':
                f = cp.linalg.norm(cp.abs(fpsi)-cp.sqrt(data))**2#+cp.sum(cp.abs(psi-lamd/rho)<1e-16)*1e16
            elif model == 'poisson':
        ...
        for i in range(piter):
            ...
            psi = psi + gamma*d                 
        #print(cp.min(cp.abs(psi-lamd/rho)))            
        return psi
    ...
    def admm(self, data, h, e, psi, phi, lamd, mu, u, alpha, piter, titer, NITER, model):
        ...
        for m in range(NITER):
            ...
            if (np.mod(m, 1) == 0):
                ...
                lagr0 = lagr[m]
                name = 'reg'+str(model)+str(piter)+str(titer)+str(NITER)
                dxchange.write_tiff(
                    u[u.shape[0]//2].imag.get(),  'betap/beta24'+name)
                dxchange.write_tiff(
                    u[u.shape[0]//2].real.get(),  'deltap/delta24'+name)                    
                dxchange.write_tiff(
                    cp.abs(psi).get(),  'psi/psi'+name)                    
        return u, psi, lagr
