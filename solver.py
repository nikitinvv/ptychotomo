        ...
        for k in range(self.theta.size):
            ...
            for m in range(len(self.scan[k].x)):
                for n in range(len(self.scan[k].y)):
                    ...
                    sty = self.scan[k].y[n]
                    phi = np.multiply(
                        self.prb.complex, psi[k][stx:stx+self.prb.size, sty:sty+self.prb.size])
                    phi = np.pad(
        ...
        for k in range(self.theta.size):
            for m in range(len(self.scan[k].x)):
                for n in range(len(self.scan[k].y)):
                    ...
                    delphi = iphi[npadx:npadx+self.prb.size,
                                  npady:npady+self.prb.size]
                    stx = self.scan[k].x[m]
                    sty = self.scan[k].y[n]
                    res[k, stx:stx+self.prb.size, sty:sty +
                        self.prb.size] += np.multiply(np.conj(self.prb.complex), delphi)
        return res
        ...
        for k in range(self.theta.size):
            for m in range(len(self.scan[k].x)):
                for n in range(len(self.scan[k].y)):
                    stx = self.scan[k].x[m]
                    sty = self.scan[k].y[n]
                    res[k, stx:stx+self.prb.size, sty:sty+self.prb.size] += np.multiply(np.abs(
                        self.prb.complex)**2, psi[k][stx:stx + self.prb.size, sty:sty + self.prb.size])
        return res
