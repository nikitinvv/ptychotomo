...
if __name__ == "__main__":
    ...
    for k in range(len(pitera)):
        ...
        name = "xdiff2_varyrho/xdiff_"+str(piter)+"_"+str(titer)+"_"+str(NITER)
        np.save(name,xdiff)
        name = "res2_varyrho/res_"+str(piter)+"_"+str(titer)+"_"+str(NITER)
        np.save(name,res)
    ...
    for k in range(len(pitera)):
        ...
        x, xdiff, res = slv.admm(
            data, h, psi, lamd, x, rho, gamma, eta, piter, titer, NITER)
        name = "xdiff2_varyrho/xdiff_noise_"+str(piter)+"_"+str(titer)+"_"+str(NITER)
        np.save(name,xdiff)
        name = "res2_varyrho/res_noise_"+str(piter)+"_"+str(titer)+"_"+str(NITER)
        np.save(name,res)