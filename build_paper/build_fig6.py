...
#############joint (rho=0.5)
        ...
        dxchange.write_tiff(
            x.beta[8],  'fig6/tiff_png/beta_joint'+name, overwrite=True)
        dxchange.write_tiff(
            x.delta[8],  'fig6/tiff_png/delta_joint'+name, overwrite=True)
...
# #############joint (rho=0)
        ...
        name = '_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
            '_maxint'+str(maxinta[k])
        dxchange.write_tiff(
            x.beta[8],  'fig6/tiff_png/beta_joint0'+name, overwrite=True)
        dxchange.write_tiff(
            x.delta[8],  'fig6/tiff_png/delta_joint0'+name, overwrite=True)
...
# #############seq
        ...
        name = '_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
            '_maxint'+str(maxinta[k])
        dxchange.write_tiff(
            x.beta[8],  'fig6/tiff_png/beta_st'+name, overwrite=True)
        dxchange.write_tiff(
            x.delta[8],  'fig6/tiff_png/delta_st'+name, overwrite=True)

    files = glob.glob('fig6/tiff_png/delta*.tiff')
    ...
    files = glob.glob('fig6/tiff_png/beta*.tiff')
    for name in files:
        a = dxchange.read_tiff(name)
        scipy.misc.toimage(a, cmin=-2.7094511e-05/4, cmax=2.7094511e-05).save(
            str(os.path.splitext(name)[0])+'.png')
    os.system(
        'for f in fig6/tiff_png/*.png; do  convert -trim "$f" "$f"; done')

    fid = open('fig6/fig6.tex', 'w')
    ...
    for k in range(0, 5):            
        name = ' & \\includegraphics[width=0.19\\textwidth]{{fig6/beta_st_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
    ...
    for k in range(0, 5):
        name = ' & \\includegraphics[width=0.19\\textwidth]{{fig6/beta_joint0_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
            '_maxint'+str(maxinta[k])+'}.png}'
        fid.write(name)
    fid.write("\\\\ \n")
    # joint
    ...
    for k in range(0, 5):
        name = ' & \\includegraphics[width=0.19\\textwidth]{{fig6/beta_joint_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
            '_maxint'+str(maxinta[k])+'}.png}'
        fid.write(name)
    fid.write("\\\\ \n")
    ...
    fid.write(" & \\rotatebox[origin=c]{90}{ standard } ")
    for k in range(0, 5):
        name = ' & \\includegraphics[width=0.19\\textwidth]{{fig6/delta_st_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
            '_maxint'+str(maxinta[k])+'}.png}'
        fid.write(name)
    fid.write("\\\\ \n")
    # joint 0
    fid.write("& \\rotatebox[origin=c]{90}{ joint $\\rho=0$ } ")
    for k in range(0, 5):
        name = ' & \\includegraphics[width=0.19\\textwidth]{{fig6/delta_joint0_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
            '_maxint'+str(maxinta[k])+'}.png}'
        fid.write(name)
    fid.write("\\\\ \n")
    # joint
    fid.write("& \\rotatebox[origin=c]{90}{ joint $\\rho=0.5$} ")
    for k in range(0, 5):
        name = ' & \\includegraphics[width=0.19\\textwidth]{{fig6/delta_joint_noise'+np.str(k > 0)+'_ang'+str(len(theta))+'_shift'+str(shift) + \
            '_maxint'+str(maxinta[k])+'}.png}'
        fid.write(name)
    fid.write("\\\\ \n")
