...
class Solver(object):
    def __init__(self, prbmaxint, nscan, nprb, theta, det, voxelsize, energy, ntheta, nz, n, ptheta, pnz):
    ...
    def cg_ptycho(self, data, init, scan, prb, h, lamd, rho, piter, model):
        # minimization functional
        def minf(psi, fpsi):
            if model == 'gaussian':
    ...
    def cg_ptycho_batch(self, data, init, scan, prb, h, lamd, rho, piter, model):
        psi = init.copy()
        for k in range(0, self.ntheta//self.ptheta):
            ids = np.arange(k*self.ptheta, (k+1)*self.ptheta)
            datap = cp.array(data[ids])*self.coefdata  # normalized data
            psi[ids] = self.cg_ptycho(
                datap, psi[ids], scan[:, ids], prb[ids], h[ids], lamd[ids], rho, piter, model)
        return psi
    ...
    def admm(self, data, h, e, psi, scan, prb, phi, lamd, mu, u, alpha, piter, titer, niter, model):
        rho = 0.5
        tau = 0.5
        for m in range(niter):
            ...
            psi = self.cg_ptycho_batch(
                data, psi, scan, prb,  h, lamd, rho, piter+(m < 2)*32, model)
            # tomography problem
