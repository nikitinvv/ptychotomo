...
class Solver(object):
    ...
    def cg_ptycho(self, data, psi, prb, scan, h, lamd, rho, piter, model, recover_prb):
        ...
        for i in range(piter):
            ...
            for k in range(self.nmodes):
                ...
                tmp2 = self.fwd_ptycho(dpsi, prb[:, k], scan)
                p1 += cp.abs(tmp1)**2
                p2 += cp.abs(tmp2)**2
            ...
            if (recover_prb):
                ...
                for m in range(0, self.nmodes):
                    ...
                    tmp2 = self.fwd_ptycho(psi, dprb[:, m], scan)
                    p2 = cp.abs(tmp2)**2
                    p3 = 2*(tmp1.real*tmp2.real+tmp1.imag*tmp2.imag)
                    # line search
                    gammaprb = self.line_search_sqr(minf, p1, p2, p3)
                    # update prb
                    prb[:, m] = prb[:, m] + gammaprb * dprb[:, m]                    
            # check convergence
            if (np.mod(i, 4) == 0):                
                print("%4d, %.3e, %.3e, %.7e, " %
    ...
    def cg_ptycho1(self, data, psi, prb, scan, h, lamd, rho, piter, model, recover_prb):
        # minimization functional
        ...
        for i in range(piter):
            ...
            # gradbefore0 = self.adj_ptycho(fpsi, prb, scan)*coef
            # gradbefore1 = -self.adj_ptycho(cp.sqrt(data)*fpsi/(cp.abs(fpsi)+1e-32), prb, scan)*coef
            
            # a=cp.sum(cp.abs(fpsi)*cp.sqrt(data))
            # b=cp.sum(cp.abs(fpsi)**2)
            ...
            coef = 1/(cp.abs(prb).max())**2
            # gradafter0 = self.adj_ptycho(fpsi, prb, scan)*coef
            # gradafter1 = -self.adj_ptycho(cp.sqrt(data)*fpsi/(cp.abs(fpsi)+1e-32), prb, scan)*coef
            # print(np.linalg.norm(gradbefore0+gradbefore1/(a/b)))
            # print(np.linalg.norm(gradafter0+gradafter1))
            # exit()
            ...
            gamma = self.line_search(minf, 1, psi, fpsi, d, fd, coef)
            psi = psi + gamma*d            
            
            print('%d psi %.2e %.2e' % (i, gamma, minf(psi, fpsi, coef)))
            ...
            if(recover_prb):
                ...
                print('%d prb %.2e %.2e' % (i, gammaprb, minfprb(prb, fprb, coef)))
                dxchange.write_tiff(cp.angle(psi).get(),  'psiiter/'+str(i), overwrite=True)                
                dxchange.write_tiff(cp.abs(psi).get(),  'psiiterabs/'+str(i), overwrite=True)                
        return psi, prb
