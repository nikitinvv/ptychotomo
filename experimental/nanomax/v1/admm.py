...
if __name__ == "__main__":    
    ...
    for k in range(ntheta):
        print('read angle', k)
        scan0 = np.load(data_prefix+'datanpy/scan128sorted_'+str(k*174//ntheta)+'.npy')        
        shifts = np.load(data_prefix+'/datanpy/shifts.npy')[k*174//ntheta]
        shifts_crop = np.load(data_prefix+'/datanpy/shifts_crop.npy')[k*174//ntheta]
        shifts_crop_sum = np.load(data_prefix+'/datanpy/shifts_crop_sum.npy')[k*174//ntheta]
        sx = (np.round(shifts[1])+shifts_crop[1]+shifts_crop_sum[1])
        sy = (np.round(shifts[0])+shifts_crop[0]+shifts_crop_sum[0])
        print(sx,sy)
        scan0[0] -= sy
        scan0[1] -= sx                                     
        scan0[1] -= 64
        ...
        data[k, :len(ids)] = np.load(
            data_prefix+'datanpy/data128sorted_'+str(k*174//ntheta)+'.npy')[ids]
    ...
    data_prefix += 'rec_sum/'+str(nscan)+'align'+str(align)+str(center)+'/'
    ...
    dxchange.write_tiff_stack(
        np.angle(psi1), data_prefix+'rec_admm_sum/psiangle/p', overwrite=True)
    dxchange.write_tiff_stack(
        np.abs(psi1), data_prefix+'rec_admm_sum/psiamp/p', overwrite=True)
    dxchange.write_tiff_stack(u.real, 'rec_admm_sum/ure/u', overwrite=True)
    dxchange.write_tiff_stack(u.imag, 'rec_admm_sum/uim/u', overwrite=True)
