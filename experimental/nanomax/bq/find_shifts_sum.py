...
if __name__ == "__main__":
    ...
    ssum = np.zeros([ntheta,400-200],dtype='float32')
    ...
    for k in range(ntheta):        
        ...
        psiangle[k] =  dxchange.read_tiff(data_prefix+'recfull_sorted_new/psiangle'+str(nmodes)+str(nscan)+'/r'+str(k)+'.tiff')  
        psiamp[k] = dxchange.read_tiff(data_prefix+'recfull_sorted_new/psiamp'+str(nmodes)+str(nscan)+'/r'+str(k)+'.tiff')          
        print(f'{data_prefix}recfull_sorted_aligned_new/r_{k:04}.tif')
        psiangle0 = dxchange.read_tiff(f'{data_prefix}recfull_sorted_aligned_new/r_{k:04}.tif')  
        #psiamp[k] = dxchange.read_tiff(f'{data_prefix}recfull_sorted_aligned_new//r_{k:04}.tiff')          
        # print(psiangle[k,30:420].shape)
        # exit()
        psiangle0 = psiangle0[200:400].copy()
    ...
    for k in range(0,ntheta):
        ...
        # print(ssum[k].shape)
        # print(ssum[0].shape)
        # shift0, error, diffphase = register_translation(ssum[k,np.newaxis], ssum[0,np.newaxis], upsample_factor=1)
        # plt.clf()
        # plt.plot(ssum[0],'r')
        # plt.plot(ssum[k],'b')
        ...
        print(f'{data_prefix}recfull_sorted_aligned_new/r_{k:04}.tif')
        psiangle0 = dxchange.read_tiff(f'{data_prefix}recfull_sorted_aligned_new/r_{k:04}.tif')  
        print(psiangle0.shape)
        print(np.linalg.norm(psiangle0))
        psiangle1 = psiangle0[200:400,128:-128].copy()
        psiangle1[psiangle1<0.0] = 0
        cm = ndimage.measurements.center_of_mass(psiangle1)
        print(cm)
        psiangle[k] = np.roll(psiangle0,-int(cm[1]-n//2+128),axis=1)    
    ...
    dxchange.write_tiff_stack(psiangle,data_prefix+'recfull_sorted_aligned_new_check/psiangle'+str(nmodes)+str(nscan)+'/r.tiff',overwrite=True)                
