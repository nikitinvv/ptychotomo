...
if __name__ == "__main__":
    ...
    shifts = np.load(data_prefix+'/datanpy/shifts_new.npy')[id_theta]
    scan0[1]-=np.round(shifts[1])
    scan0[0]-=np.round(shifts[0])
    scan0[1]-=64+30
    scan0[0]-=160
    ...
    # Save result
    dxchange.write_tiff(np.angle(psi),  data_prefix+'rec_crop_new/psiangle'+str(nmodes)+str(nscan)+'/r'+str(id_theta), overwrite=True)
    dxchange.write_tiff(np.abs(psi),   data_prefix+'rec_crop_new/psiamp'+str(nmodes)+str(nscan)+'/r'+str(id_theta), overwrite=True)
    for m in range(nmodes):
        dxchange.write_tiff(np.angle(prb[:,m]),   data_prefix+'rec_crop_new/prbangle/r'+str(id_theta), overwrite=True)
        dxchange.write_tiff(np.abs(prb[:,m]),   data_prefix+'rec_crop_new/prbamp/r'+str(id_theta), overwrite=True)
