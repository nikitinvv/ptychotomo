...
if __name__ == "__main__":
    ...
    scan0[1]-=np.round(shifts[1]+shiftspart[1]+shiftssum[1])
    ...
    # Save result
    dxchange.write_tiff(np.angle(psi),  data_prefix+'reccropsum/psiangle'+str(nmodes)+str(nscan)+'/r'+str(id_theta), overwrite=True)
    dxchange.write_tiff(np.abs(psi),   data_prefix+'reccropsum/psiamp'+str(nmodes)+str(nscan)+'/r'+str(id_theta), overwrite=True)
    for m in range(nmodes):
        dxchange.write_tiff(np.angle(prb[:,m]),   data_prefix+'reccropsum/prbangle/r'+str(id_theta), overwrite=True)
        dxchange.write_tiff(np.abs(prb[:,m]),   data_prefix+'reccropsum/prbamp/r'+str(id_theta), overwrite=True)
