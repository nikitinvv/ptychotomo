...
if __name__ == "__main__":    
    ...
    # data0 = np.load(
            # data_prefix+'datanpy/data128sorted_'+str(0)+'.npy')
    for k in range(ntheta):
        print('read angle', k)
        scan0 = np.load(data_prefix+'datanpy/scan128sorted_'+str(k*166//ntheta)+'.npy')        
        if(shift_type=='stxm'):
            scan0[0] += np.load('sx_stxm_new2.npy')[k*166//ntheta]
            scan0[1] += np.load('sy_stxm_new2.npy')[k*166//ntheta]
        else:
            ...
            print(shifts[0],shiftscrop[0],shifts[1],shiftscrop[1])
            scan0[0] -= np.round(shifts[0]+shiftscrop[0])
            scan0[1] -= np.round(shifts[1]+shiftscrop[1])            
    ...
    data_prefix += 'rec/'+str(nscan)+shift_type+str(center)+'/'
