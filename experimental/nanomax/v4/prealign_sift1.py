...
for k in range(ntheta):
    img0[k] = dxchange.read_tiff(data_prefix+'rec_full/psiangle'+str(nmodes)+str(nscan)+'/r'+str(k)+'.tiff')
...
dxchange.write_tiff_stack(imgres.astype('float32'),data_prefix+'prealign_sift1/r.tiff',overwrite=True)
print(shifts)
np.save(data_prefix+'/datanpy/shifts1_'+str(nscan)+'.npy',shifts)
