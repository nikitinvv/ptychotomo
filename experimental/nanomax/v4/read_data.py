...
if __name__ == "__main__":   
    ...
    if not os.path.exists(data_prefix+'datanpy'):
        os.makedirs(data_prefix+'datanpy')
    if not os.path.exists(data_prefix+'scan_pos'):
        os.makedirs(data_prefix+'scan_pos')
    if not os.path.exists(data_prefix+'tmp'):
        os.makedirs(data_prefix+'tmp')        
    ...
    for k in range(134, 424, 1):
        ...
        if(scan is not None):            
            ...
            plt.axis('equal')
            plt.savefig(data_prefix+'/scan_pos/'+str(kk)+'.png',dpi=200)
            if kk>=166:
                scan[0]+=166
            print('theta',theta)
