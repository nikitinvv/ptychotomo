...
if __name__ == "__main__":
    ...
    shifts3 = np.load(data_prefix+'/datanpy/shifts3_'+str(nscan)+'.npy')[id_theta]
    ...
    for m in range(nmodes):
        dxchange.write_tiff(np.angle(prb[:,m]),   data_prefix+'rec_crop3/prbangle/r'+str(id_theta), overwrite=True)
        dxchange.write_tiff(np.abs(prb[:,m]),   data_prefix+'rec_crop3/prbamp/r'+str(id_theta), overwrite=True)
