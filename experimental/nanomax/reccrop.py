...
if __name__ == "__main__":
    ...
    # Save result
    dxchange.write_tiff(np.angle(psi),  data_prefix+'reccrop/psiangle' +
                        '/r'+str(id_theta), overwrite=True)
    dxchange.write_tiff(np.abs(psi),   data_prefix+'reccrop/psiamp' +
                        '/r'+str(id_theta), overwrite=True)
    for m in range(nmodes):
        dxchange.write_tiff(np.angle(
            prb[:, m]),   data_prefix+'reccrop/prbangle/r'+str(id_theta), overwrite=True)
        dxchange.write_tiff(np.abs(
            prb[:, m]),   data_prefix+'reccrop/prbamp/r'+str(id_theta), overwrite=True)
