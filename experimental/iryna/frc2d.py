...
f1 = dxchange.read_tiff_stack(fname1, ind = np.arange(512,513))
f2 = dxchange.read_tiff_stack(fname2, ind = np.arange(512,513))
...
frc3 = radial_profile(ff1*np.conj(ff2),np.array(ff1.shape)//2)/\
    np.sqrt(radial_profile(np.abs(ff1)**2,np.array(ff1.shape)//2)*radial_profile(np.abs(ff2)**2,np.array(ff1.shape)//2))
np.save('frc32d.npy',frc3)
# 


hbit = halfbit(ff1,np.array(ff1.shape)//2)
np.save('hbit2d.npy',hbit)
# exit()
# wsize = 512
frc1=np.load('frc12d.npy')
frc2=np.load('frc22d.npy')
frc3=np.load('frc32d.npy')
