...
if __name__ == "__main__":
    ...
    maxint = 1  # maximal probe intensity    
    ...
    alpha = 1e-7  # tv regularization penalty coefficient
    ...
    # Load a 3D object
    beta = dxchange.read_tiff('data/BETA256.tiff')[16:16+32].astype('float32')
    delta = dxchange.read_tiff('data/DELTA256.tiff')[16:16+32].astype('float32')    


    # Create object, probe, angles, scan positions
    obj = cp.array(delta+1j*beta)
    prb = cp.array(objects.probe(prbsize, maxint))#,rout=1.03))  # ,rout=1.03))
